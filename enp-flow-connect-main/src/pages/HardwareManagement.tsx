...
import { Cpu, Radio, Plus, Settings, Activity, CheckCircle, XCircle } from "lucide-react";
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="shadow-card">
            <CardHeader>
              ...
              <CardDescription>Real-time hardware connectivity</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {hardwareDevices.slice(0, 3).map((device) => (
          ...
          <Card className="shadow-card">
            <CardHeader>
              ...
              <CardDescription>Active sensor configurations</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div>
                    <p className="font-medium">RFID Sensors</p>
                    <p className="text-sm text-muted-foreground">13.56 MHz NFC Readers</p>
                  </div>
                  <Badge className="bg-primary text-primary-foreground">
                    {hardwareDevices.filter(d => d.type === "RFID Reader").length} Active
                  </Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div>
                    <p className="font-medium">Motion Sensors</p>
                    <p className="text-sm text-muted-foreground">PIR & Ultrasonic</p>
                  </div>
                  <Badge className="bg-primary text-primary-foreground">6 Active</Badge>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div>
                    <p className="font-medium">Barrier Controllers</p>
                    <p className="text-sm text-muted-foreground">Servo Motor Control</p>
                  </div>
                  <Badge className="bg-primary text-primary-foreground">4 Active</Badge>
                </div>
              </div>
            </CardContent>
          </Card>
